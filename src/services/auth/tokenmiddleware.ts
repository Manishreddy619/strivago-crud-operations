...
import customerModel from "../customers/schema";
import { verifyJWT } from "./tools";

import { Users, Accommodation } from "../../interfaces";
interface idd {
  id: string;
}
const users: idd[] = [];

export const addUser = (newToken: idd) => {
  users.push(newToken);
};
export const JWTAuthMiddleware = async (req, res, next) => {
  ...
  if (!req.headers.authorization) {
    ...
    try {
      ...
      const decodedToken: idd[] = await verifyJWT(token);
      console.log("DECODED TOKEN ", decodedToken);

      // 4. Find the user in db and attach him/her to the request object
      const user = await customerModel.findById(decodedToken._id);
